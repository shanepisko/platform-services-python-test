...
class MyTestCase(AsyncTestCase):
    ...
    def handle_fetch(self, response):
        ...
        print(response.body)
        self.assertLess(0, len(response.body))
        self.stop()
...
class UpdateUserTestCase(AsyncTestCase):
    ...
    def delete_test_user(self):
        client = AsyncHTTPClient(self.io_loop)
        # body = urllib.parse.urlencode(data) #Make it into a post request
        client.fetch("http://localhost:7050/user-rewards/hondo@gmail.com", self.handle_delete, method='DELETE', headers=None, body=None)
        self.wait()
